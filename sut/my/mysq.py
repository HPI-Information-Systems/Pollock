...
def load_csv(filename, params):
    ...
    out_tmp_header_path = join('/tmp/', filename + '_data.csv').replace('\\', '/')
    out_tmp_data_path = join('/tmp/', filename + '_header.csv').replace('\\', '/')
    ...
    out_query_header = f"""
                select GROUP_CONCAT(CONCAT('"',COLUMN_NAME,'"'))
                        ...
                        LINES TERMINATED BY '\\r\\n';"""
    out_query_data = f"""
            SELECT * from table1 INTO OUTFILE '{out_tmp_data_path}'
    ...
    try:
        ...
        print("\t", "Error in query:", out_query_data)
        print(e, "\n")

    sleep(0.5)
    Path(OUT_DIR).mkdir(parents=True, exist_ok=True)

    cat_cmd = f'cat {out_tmp_header_path} {out_tmp_data_path} > {out_path}'
    os.system(cat_cmd)
